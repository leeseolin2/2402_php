...
for($i = 1; $i < 7; $i++){
    $print_n = random_int(1, 45)." ";
    $print_s = random_int(1, 45)."\n";
    if( $print_s === $print_n){ 
       continue;
    }
...
$numbers = array_rand(range(1, 45), 6);
sort($numbers);
echo "로또 번호: " . implode(", ", $numbers);
...
$arr_pick = []; // 뽑은 값 저장용
for($i = 0; $i < 6; $i++) {
    $int_rand = random_int(1, count($arr_base) -1); // 랜덤 숫자 획득(배열의 키)
