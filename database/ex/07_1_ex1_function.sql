...
-- 제어 흐름 함수
-- IF(수식, 참일 떄, 거짓일 때) : 수식이 참이면 참일때, 거짓이면 거짓일때 출력
SELECT emp_no, gender , IF(gender = 'M','남자','여자')
FROM employees;
-- 급여가 8000 이상인 사원은 '고소득자' 아니면 '그냥저냥'으로 출력
...
SELECT UPPER('asdgDD'), LOWER('FASDsaft');
...
SELECT
	...
	,salary
	,RANK() OVER(ORDER BY salary DESC) AS sal_rank
	,ROW_NUMBER() OVER(ORDER BY salary DESC) AS sal_rown
FROM salaries
WHERE to_date >= NOW()
LIMIT 5;          
